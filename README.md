...
<body>
  ...
  resultDiv.innerHTML = `
    ...
    <p>لقد أجبت بشكل صحيح على ${correctCount} من ${questions.length} سؤال.</p>
...
function showQuestion() {
  ...
  q.choices.forEach((choice, index) => {
    ...
    input.onclick = () => userAnswers[currentQuestion] = index;
...
function calculateResult() {
  ...
  resultDiv.innerHTML = `
    ...
    <h3>النتيجة النهائية: ${score} / ${questions.length}</h3>

    <p>${score >= questions.length / 2 ? '✅ لقد نجحت في الاختبار' : '❌ لقد رسبت في الاختبار'}</p>
